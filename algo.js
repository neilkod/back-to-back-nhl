...
/* ---- the streak sweep: single left-to-right pass, O(number of game-days).
   ...
   Carrying the frontier forward catches that (e.g. a forced West-Coast
   LA/ANA shuttle that can't actually bridge to the Eastern corridor even
   though *some other* team happens to play a bridging game that same night). ---- */
...
/* ==================== dev checks ====================
   ...
   a "21-night run, Jan 8-28" at the 4.0h stop — but that span isn't actually
   travelable as one continuous itinerary. It only looks connected under a
   naive "some pair of teams links these two nights" test; the specific team
   your itinerary would be riding (forced through the LA/ANA shuttle, since
   that's the only linkable pair across Jan 8-14) can never reach the Jan 15
   Islanders game (ANA-NYI has no travel entry at all). findRunDateGroups()
   here carries a reachability "frontier" forward instead of testing pairs in
   isolation, which correctly splits that into a 7-night West-Coast run
   (Jan 8-14) and a 14-night corridor run (Jan 15-28) — the true longest run
   at every stop, including 4.0h, is 14 nights. */
(function devChecks() {
